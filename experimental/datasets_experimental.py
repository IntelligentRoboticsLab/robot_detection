...
class RoboEireanData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(target_bounding_boxes_list)

        if 'train' in self.data_path and target_bounding_boxes_list:
            ...
            album_bboxes = []
            for tensor, target_class in zip(target_bounding_boxes, target_classes):

                album_bboxes.append(torch.cat((tensor, target_class.unsqueeze(dim=0))))

            # print(album_bboxes)

            # print("-------------------------")
            if self.image_transforms:
                augment = self.image_transforms(
                    image=image, bboxes=album_bboxes
                    )
                image = augment['image']
                # print(augment['bboxes'][0][-1])
                bboxes = list(augment['bboxes'][0][:4])
                target_bounding_boxes = torch.tensor([bboxes])

            

            image = pytorch_transform(Image.fromarray(image))
...
class RoboEireanDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.train_transforms = A.Compose(
            [
        A.RandomSizedBBoxSafeCrop(height=640, width=480),
